...
#Witsarut Janthornsirijan 61130067
...
def check(lat, lon, baro, callsign):
	...
	for x in range(len(lat)):
		for y in range(len(lat)-(x+1)):
			# 1 meters = 3.28084 ft.
			if(abs(baro[x]-baro[x+y+1])*3.28084 < 1000):
				if(horiz_dist(lat[x], lat[x+y+1], lon[x], lon[x+y+1]) < (3*1.852)):
...
print('All conflict is ' + str(len(ans)) + ' times')
#print(confl)
#print(confl_t)
